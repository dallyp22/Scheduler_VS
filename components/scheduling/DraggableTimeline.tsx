'use client';

import React, { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Tooltip, Tag, App } from 'antd';
import type { ScheduleBlock, ProductionLine } from '@/types';
import { BlockStatus } from '@/types/enums';

interface DraggableTimelineProps {
  blocks: ScheduleBlock[];
  lines: ProductionLine[];
  startDate: Date;
  endDate: Date;
  onBlockMove: (blockId: string, newLineId: string, newStartTime: Date) => void;
}

const HOUR_WIDTH = 60;
const LINE_HEIGHT = 80;
const HEADER_HEIGHT = 40;
const SIDEBAR_WIDTH = 200;

export default function DraggableTimeline({
  blocks,
  lines,
  startDate,
  endDate,
  onBlockMove,
}: DraggableTimelineProps) {
  const { message } = App.useApp();
  const [draggedBlock, setDraggedBlock] = useState<string | null>(null);
  const [hoveredBlock, setHoveredBlock] = useState<string | null>(null);
  
  const activeLines = lines.filter(line => line.status === 'active');
  const hours = 24;
  const timelineWidth = hours * HOUR_WIDTH;
  const timelineHeight = activeLines.length * LINE_HEIGHT;

  const hourMarkers = Array.from({ length: hours + 1 }, (_, i) => ({
    hour: i,
    x: i * HOUR_WIDTH,
  }));

  const getBlockPosition = (block: ScheduleBlock, lineIndex: number) => {
    const blockStart = block.startTime.getTime();
    const timelineStart = startDate.getTime();
    const hoursFromStart = (blockStart - timelineStart) / (1000 * 60 * 60);
    
    const x = hoursFromStart * HOUR_WIDTH;
    const y = lineIndex * LINE_HEIGHT + 15;
    const width = (block.duration / 60) * HOUR_WIDTH;
    const height = LINE_HEIGHT - 30;
    
    return { x, y, width, height };
  };

  const getStatusColor = (status: BlockStatus): string => {
    const colors: Record<BlockStatus, string> = {
      [BlockStatus.PLANNED]: '#1890ff',
      [BlockStatus.READY]: '#13c2c2',
      [BlockStatus.CHANGEOVER]: '#faad14',
      [BlockStatus.RUNNING]: '#52c41a',
      [BlockStatus.COMPLETED]: '#95de64',
      [BlockStatus.DELAYED]: '#ff4d4f',
    };
    return colors[status] || '#d9d9d9';
  };

  const handleDragStart = (blockId: string) => {
    setDraggedBlock(blockId);
  };

  const handleDragEnd = () => {
    if (draggedBlock) {
      message.success('Block moved! Recalculating schedule...', 2);
      setDraggedBlock(null);
    }
  };

  return (
    <div style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: '700px', position: 'relative' }}>
      <div style={{ position: 'relative', width: timelineWidth + SIDEBAR_WIDTH, height: timelineHeight + HEADER_HEIGHT }}>
        {/* Time header */}
        <div 
          style={{
            position: 'sticky',
            top: 0,
            left: 0,
            zIndex: 3,
            background: '#fafafa',
            borderBottom: '2px solid #d9d9d9',
          }}
        >
          <div style={{ display: 'flex' }}>
            <div style={{ width: SIDEBAR_WIDTH, flexShrink: 0, borderRight: '1px solid #d9d9d9' }}>
              <div style={{ padding: '8px', fontWeight: 600, textAlign: 'center' }}>
                Production Lines
              </div>
            </div>
            <div style={{ flex: 1 }}>
              <svg width={timelineWidth} height={HEADER_HEIGHT}>
                {hourMarkers.map((marker, i) => (
                  <g key={i}>
                    <line
                      x1={marker.x}
                      y1={0}
                      x2={marker.x}
                      y2={HEADER_HEIGHT}
                      stroke="#d9d9d9"
                      strokeWidth={marker.hour % 6 === 0 ? 2 : 1}
                    />
                    {marker.hour % 2 === 0 && (
                      <text
                        x={marker.x + 5}
                        y={HEADER_HEIGHT / 2 + 5}
                        fontSize="12"
                        fill="#666"
                      >
                        {marker.hour}:00
                      </text>
                    )}
                  </g>
                ))}
              </svg>
            </div>
          </div>
        </div>

        {/* Timeline body */}
        <div style={{ display: 'flex' }}>
          {/* Line labels (sticky) */}
          <div
            style={{
              width: SIDEBAR_WIDTH,
              flexShrink: 0,
              position: 'sticky',
              left: 0,
              zIndex: 2,
              background: '#fff',
              borderRight: '1px solid #d9d9d9',
            }}
          >
            {activeLines.map((line, index) => (
              <motion.div
                key={line.id}
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: index * 0.05 }}
                style={{
                  height: LINE_HEIGHT,
                  padding: '12px',
                  borderBottom: '1px solid #f0f0f0',
                  display: 'flex',
                  flexDirection: 'column',
                  justifyContent: 'center',
                }}
              >
                <div style={{ fontWeight: 600 }}>{line.code}</div>
                <div style={{ fontSize: '11px', color: '#666', marginTop: 4 }}>
                  OEE: {((line.metrics.currentOEE || 0) * 100).toFixed(0)}%
                </div>
                <Tag color={line.status === 'active' ? 'green' : 'default'} style={{ marginTop: 4, width: 'fit-content', fontSize: '10px' }}>
                  {line.status}
                </Tag>
              </motion.div>
            ))}
          </div>

          {/* Timeline grid and blocks */}
          <div style={{ flex: 1, position: 'relative' }}>
            <svg width={timelineWidth} height={timelineHeight}>
              {/* Grid */}
              {hourMarkers.map((marker, i) => (
                <line
                  key={`grid-${i}`}
                  x1={marker.x}
                  y1={0}
                  x2={marker.x}
                  y2={timelineHeight}
                  stroke="#f0f0f0"
                  strokeWidth={marker.hour % 6 === 0 ? 2 : 1}
                />
              ))}
              
              {activeLines.map((_, index) => (
                <line
                  key={`line-${index}`}
                  x1={0}
                  y1={(index + 1) * LINE_HEIGHT}
                  x2={timelineWidth}
                  y2={(index + 1) * LINE_HEIGHT}
                  stroke="#f0f0f0"
                  strokeWidth={1}
                />
              ))}

              {/* Blocks */}
              <AnimatePresence>
                {blocks.map(block => {
                  const lineIndex = activeLines.findIndex(l => l.id === block.lineId);
                  if (lineIndex === -1) return null;
                  
                  const pos = getBlockPosition(block, lineIndex);
                  const color = block.color || getStatusColor(block.status);
                  const isDragged = draggedBlock === block.id;
                  const isHovered = hoveredBlock === block.id;
                  
                  return (
                    <motion.g
                      key={block.id}
                      initial={{ scale: 0.8, opacity: 0 }}
                      animate={{ scale: 1, opacity: isDragged ? 0.5 : 1 }}
                      exit={{ scale: 0.8, opacity: 0 }}
                      transition={{ duration: 0.3 }}
                      onMouseEnter={() => setHoveredBlock(block.id)}
                      onMouseLeave={() => setHoveredBlock(null)}
                      style={{ cursor: 'grab' }}
                    >
                      <rect
                        x={pos.x}
                        y={pos.y}
                        width={pos.width}
                        height={pos.height}
                        fill={color}
                        stroke={isHovered ? '#000' : '#fff'}
                        strokeWidth={isHovered ? 3 : 1}
                        rx={4}
                        opacity={0.85}
                        onMouseDown={() => handleDragStart(block.id)}
                        onMouseUp={handleDragEnd}
                      >
                        <title>
                          {block.order?.sku.code}
                          {'\n'}Status: {block.status}
                          {'\n'}Duration: {block.duration} min
                          {block.changeoverMinutes && `\nChangeover: ${block.changeoverMinutes} min`}
                        </title>
                      </rect>
                      {pos.width > 60 && (
                        <text
                          x={pos.x + 8}
                          y={pos.y + pos.height / 2 + 5}
                          fontSize="11"
                          fill="#fff"
                          fontWeight="500"
                          pointerEvents="none"
                        >
                          {block.order?.sku.code}
                        </text>
                      )}
                      {/* Animated progress indicator for running blocks */}
                      {block.status === BlockStatus.RUNNING && (
                        <motion.rect
                          x={pos.x}
                          y={pos.y}
                          width={pos.width}
                          height={4}
                          fill="#52c41a"
                          initial={{ width: 0 }}
                          animate={{ width: pos.width }}
                          transition={{ duration: 2, repeat: Infinity }}
                        />
                      )}
                    </motion.g>
                  );
                })}
              </AnimatePresence>
              
              {/* Current time indicator with pulse animation */}
              {(() => {
                const now = new Date();
                const hoursFromStart = (now.getTime() - startDate.getTime()) / (1000 * 60 * 60);
                const x = hoursFromStart * HOUR_WIDTH;
                
                if (x >= 0 && x <= timelineWidth) {
                  return (
                    <>
                      <motion.line
                        x1={x}
                        y1={0}
                        x2={x}
                        y2={timelineHeight}
                        stroke="#ff4d4f"
                        strokeWidth={2}
                        strokeDasharray="4"
                        animate={{ opacity: [1, 0.5, 1] }}
                        transition={{ duration: 2, repeat: Infinity }}
                      />
                      <motion.circle
                        cx={x}
                        cy={10}
                        r={6}
                        fill="#ff4d4f"
                        animate={{ scale: [1, 1.2, 1] }}
                        transition={{ duration: 1.5, repeat: Infinity }}
                      />
                    </>
                  );
                }
                return null;
              })()}
            </svg>
          </div>
        </div>
      </div>
    </div>
  );
}
